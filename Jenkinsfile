node(label: 'raspberrypi') {
    ...
    def dists = ["bookworm" ,"bullseye", "buster", "stretch"]
    ...
    for (int i = 0; i < dists.size(); ++i) {
        ...
        stage("Test install on ${dist}") {
            sh "BRANCH=${env.BRANCH_NAME} /build/pi-builder/scripts/validate-packages.sh ${dist} ${results}/piaware-gui_*.deb ${results}/piaware-gui-release*.deb"
        }
    }
